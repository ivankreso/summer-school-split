...
def validate(data, x, y, y_pred, loss, conf_mat):
  print('\nValidation phase:')
  conf_mat_all = np.zeros((num_classes, num_classes), dtype=np.uint64) 
  for i, (x_np, y_np, names) in enumerate(data):
    ...
    if i % 10 == 0:
      string = 'batch %d loss = %.2f (%.1f examples/sec)' % \
        (i, loss_np, x_np.shape[0] / duration)
      print(string)
  print(conf_mat)
  return utils.print_metrics(conf_mat_all, 'Validation', Dataset.class_info)
...
y_vec_pred = tf.argmax(logits_vec, axis=1, output_type=tf.int32)
y_pred = tf.argmax(logits, axis=3, output_type=tf.int32)
conf_mat = tf.confusion_matrix(y_vec, y_vec_pred, num_classes)
...
train_writer = tf.summary.FileWriter('assets/logs/train', sess.graph)
...
for epoch in range(1, num_epochs+1):
  ...
  iou = validate(val_data, x, y, y_pred, loss, conf_mat)
  if iou > best_iou:
